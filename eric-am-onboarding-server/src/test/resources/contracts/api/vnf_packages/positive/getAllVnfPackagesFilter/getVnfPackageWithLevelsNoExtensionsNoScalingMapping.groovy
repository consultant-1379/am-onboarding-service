 ...
 */
package contracts.api.vnf_packages.positive.getAllVnfPackagesFilter
...
""")
        ...
        url("/api/vnfpkgm/v1/vnf_packages") {
            queryParameters {
                parameter 'filter': value(consumer(regex("^\\((?:eq|neq|in|nin|gt|gte|lt|lte|cont|ncont)," +
                        ...
                        "levels-no-extensions-no-scaling-mapping\\)\$")),
                        producer("(eq,id,levels-no-extensions-no-scaling-mapping)"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file("filteredVnfPackageWithLevelsNoExtensionsNoScalingMapping.json").asString())
        bodyMatchers {
