...
""")
    request {
        ...
        url("/api/vnfpkgm/v1/vnf_packages") {
            queryParameters {
                parameter 'filter': value(consumer(regex("^\\((?:eq|neq|in|nin|gt|gte|lt|lte|cont|ncont),(?:id|vnfdId|vnfProvider|vnfProductName|vnfSoftwareVersion|vnfdVersion|onboardingState|operationalState|usageState),([a-zA-Z0-9- ]+)\\)\$")),
                        producer("(eq,id,d3def1ce-4cf4-477c-aab3-21cb04e6a379)"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file("filteredvnfpackages.json").asString())
        bodyMatchers {
            ...
            jsonPath('$.[0].packageSecurityOption', byRegex("OPTION_1|OPTION_2|UNSIGNED"))
