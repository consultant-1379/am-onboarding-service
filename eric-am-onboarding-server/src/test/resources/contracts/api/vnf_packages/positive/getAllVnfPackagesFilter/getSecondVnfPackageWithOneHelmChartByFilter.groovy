...
""")
    request {
        ...
        url("/api/vnfpkgm/v1/vnf_packages") {
            queryParameters {
                parameter 'filter': value(consumer(regex("^\\((?:eq|neq|in|nin|gt|gte|lt|lte|cont|ncont),(?:id|vnfdId|vnfProvider|vnfProductName|vnfSoftwareVersion|vnfdVersion|onboardingState|operationalState|usageState),single-helm-chart-2\\)\$")),
                        producer("(eq,id,single-helm-chart-2)"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file("secondvnfpackagesOneHelmChart.json").asString())
        bodyMatchers {
