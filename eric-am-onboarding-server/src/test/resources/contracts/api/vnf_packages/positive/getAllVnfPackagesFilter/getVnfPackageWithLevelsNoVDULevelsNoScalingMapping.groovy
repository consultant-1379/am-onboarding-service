...
""")
        ...
        url("/api/vnfpkgm/v1/vnf_packages") {
            queryParameters {
                parameter 'filter': value(consumer(regex("^\\((?:eq|neq|in|nin|gt|gte|lt|lte|cont|ncont)," +
                        ...
                        "levels-no-vdu-no-scaling-mapping\\)\$")),
                        producer("(eq,id,levels-no-vdu-no-scaling-mapping)"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file("filteredVnfPackageWithLevelsNoVDULevelsNoScalingMapping.json").asString())
        bodyMatchers {
