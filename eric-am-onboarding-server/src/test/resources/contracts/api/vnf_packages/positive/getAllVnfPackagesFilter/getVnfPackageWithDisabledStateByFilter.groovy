...
Contract.make {
    description("""
Returns successful scenario for onboarded vnf packages in disabled state
...
then:
  a vnf package in disabled state is returned
```
...
""")
        ...
        url("/api/vnfpkgm/v1/vnf_packages") {
            queryParameters {
                parameter 'filter': value(consumer(regex("^\\((?:eq|neq|in|nin|gt|gte|lt|lte|cont|ncont)," +
                        ...
                        "multi-helm-chart-disabled\\)\$")),
                        producer("(eq,vnfdId,multi-helm-chart-disabled)"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file("filteredVnfPackagesWithDisabledState.json").asString())
        bodyMatchers {
