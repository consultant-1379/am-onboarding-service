 ...
 */
package contracts.api.vnf_packages.positive.createVnfPkgInfoRequestWithUserData

import org.springframework.cloud.contract.spec.Contract

Contract.make {
    description("""
Successful scenario for creation of an individual VNF package resource with userDefinedData.

```
given:
  client requests to create a VNF package resource using a data structure of type CreateVnfPkgInfoRequest in the payload body
when:
...
""")
    request {
        ...
        bodyMatchers {
            jsonPath('$.userDefinedData', byRegex(/([\t\n\r\s]*.)*/))
        }
    }
    response {
        status CREATED()
        ...
        body("""
                {
                    ...
                    "userDefinedData": "${fromRequest().body('$.userDefinedData')}",
                    "_links":{
                      "self": {
        ...
        bodyMatchers {
            ...
            jsonPath('$.userDefinedData', byCommand("assertThat(parsedJson.read(\"\$.userDefinedData\", Object.class)).isNotNull()"))
