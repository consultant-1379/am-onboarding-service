...
""")
    request{
        method GET()
        url "/api/vnfpkgm/v1/vnf_packages/${value(consumer(anyNonEmptyString()))}/artifacts/${value(consumer(anyNonEmptyString()))}plain"
        headers {
            accept(textPlain())
        }
    }
    response {
        ...
        body(file("sample_vnfd.yaml.properties"))
