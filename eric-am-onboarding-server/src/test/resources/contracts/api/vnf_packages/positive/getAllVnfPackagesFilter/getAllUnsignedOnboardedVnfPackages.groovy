...
when:
  a valid request is made to retrieve all of the unsigned onboarded vnf packages
then:
        ...
        url("/api/vnfpkgm/v1/vnf_packages") {
            queryParameters {
                parameter 'filter': value(consumer(regex("^\\((?:eq|neq|in|nin|gt|gte|lt|lte|cont|ncont)," +
                        ...
                        "|packageSecurityOption),([a-zA-Z0-9- ]+)\\)\$")),
                        producer("(eq,packageSecurityOption,UNSIGNED)"))
            }
        }
        headers {
