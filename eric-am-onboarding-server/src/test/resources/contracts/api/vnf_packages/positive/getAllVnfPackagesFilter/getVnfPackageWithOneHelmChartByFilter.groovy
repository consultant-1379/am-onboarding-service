...
""")
    request {
        ...
        url("/api/vnfpkgm/v1/vnf_packages") {
            queryParameters {
                parameter 'filter': value(consumer(regex("^\\((?:eq|neq|in|nin|gt|gte|lt|lte|cont|ncont),(?:id|vnfdId|vnfProvider|vnfProductName|vnfSoftwareVersion|vnfdVersion|onboardingState|operationalState|usageState),single-helm-chart\\)\$")),
                        producer("(eq,id,single-helm-chart)"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file("filteredvnfpackagesWithOneHelmChart.json").asString())
        bodyMatchers {
