...
Contract.make {
    description("""
Returns successful scenario for onboarded vnf packages
...
""")
        ...
        url("/api/vnfpkgm/v1/vnf_packages") {
            queryParameters {
                parameter 'filter': value(consumer(regex("^\\((?:eq|neq|in|nin|gt|gte|lt|lte|cont|ncont)," +
                        ...
                        "multi-rollback-4cf4-477c-aab3-21cb04e6b\\)\$")),
                        producer("(eq,vnfdId,multi-rollback-4cf4-477c-aab3-21cb04e6b)"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file("filteredVnfPackagesWithRollback.json").asString())
        bodyMatchers {
